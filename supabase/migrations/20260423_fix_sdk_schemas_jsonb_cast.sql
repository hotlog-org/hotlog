-- Fix: fields.metadata is json (not jsonb), so COALESCE with '{}'::jsonb errored.
-- Cast it explicitly so the whole result graph is jsonb.
          ...
          'fields', COALESCE((
            SELECT jsonb_agg(
              jsonb_build_object(
                ...
                'type', f.type,
                'required', f.required,
                'metadata', COALESCE(f.metadata::jsonb, '{}'::jsonb)
              )
              ORDER BY f.key
            )
