...
export default function HomeClient({ translations }: Props) {
  ...
  const handleGetStarted = () => {
    blackSectionRef.current?.scrollIntoView({
      behavior: 'smooth',
      block: 'start',
    })
    setAnimate(true)
  ...
  return (
    <>
      <section
        className={cn(
          'min-h-screen flex flex-col justify-center items-center px-6 py-12 text-center',
          styles.section,
        )}
      >
        <h1
          className={cn(
            'text-[3.2rem] font-black mb-4 tracking-[1.5px]',
            styles.homeTitle,
          )}
        >
          {t('welcome')}
        ...
        <button
          className='px-10 py-3 text-2xl font-bold text-white bg-[#2C112B] 
      ...
      >
        <h2 className={'text-[2.2rem] font-extrabold mb-4 tracking-wide'}>
          {t('blackSectionTitle')}
        </h2>
