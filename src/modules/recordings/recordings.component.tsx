...
import { DonutChart, LineChart, type BaseChartData, type TimeSeriesData } from '@/shared/charts'
...
import { AlertCircle, CheckCircle2, ChevronDown, ChevronRight, Clock, Copy, Search, XCircle } from 'lucide-react'
...
const generateSampleLogs = (): ApiLog[] => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const errorMessage = isError ? errors[Math.floor(Math.random() * errors.length)] : undefined
    logs.push({
      id: `log-${i}`,
      ...
      requestBody: method !== 'GET' ? JSON.stringify({ userId: Math.floor(Math.random() * 1000), action: 'process' }, null, 2) : undefined,
      responseBody: isError ? JSON.stringify({ error: errorMessage, code: statusCode }, null, 2) : JSON.stringify({ success: true, data: { id: Math.floor(Math.random() * 1000) } }, null, 2),
      stackTrace: isError ? `Error: ${errorMessage}\n    at processRequest (handler.js:45)\n    at handleAPI (router.js:123)\n    at middleware (app.js:67)` : undefined,
    })
  }
...
export function RecordingsComponent() {
  ...
  const stats = useMemo(() => {
    ...
    const success = logs.filter((l) => l.statusCode >= 200 && l.statusCode < 400).length
    ...
    const errorRate = (((clientErrors + serverErrors) / total) * 100).toFixed(1)
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">All recorded requests</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{stats.success} successful requests</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{stats.clientErrors + stats.serverErrors} failed requests</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
                    ...
                    return (
                      <>
                        <TableRow
                          key={log.id}
                          className={cn(
                            log.statusCode >= 400 && 'bg-destructive/5 hover:bg-destructive/10',
                            'cursor-pointer'
                          )}
                        ...
                        >
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              ...
                              <span className={cn(
                                log.responseTime > 1000 && 'text-yellow-600 dark:text-yellow-400',
                                log.responseTime > 2000 && 'text-red-600 dark:text-red-400'
                              )}>
                                {log.responseTime}ms
                              </span>
                            </div>
                              ...
                              <div className="grid gap-4 md:grid-cols-2">
                                  ...
                                  {log.requestBody && (
                                    <div className="mt-3">
                                      <div className="flex items-center justify-between mb-1">
                                        <h5 className="text-xs font-semibold">Request Body</h5>
                                        <Button
                                          variant="ghost"
                                          size="sm"
                                ...
                                <div>
                                  ...
                                  {log.responseBody && (
                                    <div className="mb-3">
                                      <div className="flex items-center justify-between mb-1">
                                        <h5 className="text-xs font-semibold">Response Body</h5>
                                        <Button
                                          variant="ghost"
                                          size="sm"
                                          className="h-6 text-xs"
                                  ...
                                  {log.stackTrace && (
                                    <div className="mt-3">
                                      <div className="flex items-center justify-between mb-1">
                                        <h5 className="text-xs font-semibold text-red-600 dark:text-red-400">Stack Trace</h5>
                                        <Button
                                          variant="ghost"
                                          size="sm"
                                          className="h-6 text-xs"
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-xs">{formatTime(selectedLog.timestamp)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground">Trace ID:</span>
                      <div className="flex items-center gap-1">
                        <span className="font-mono text-xs">{selectedLog.traceId}</span>
                        <Button
                          variant="ghost"
                          size="sm"
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono mt-1 break-all">{selectedLog.userAgent}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="h-6 text-xs"
                      onClick={() => copyToClipboard(JSON.stringify(selectedLog.requestHeaders, null, 2))}
                    >
                      <Copy className="w-3 h-3 mr-1" />
                      Copy
                    </Button>
              ...
              {selectedLog.stackTrace && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-semibold text-red-600 dark:text-red-400">Stack Trace</h4>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 text-xs"
