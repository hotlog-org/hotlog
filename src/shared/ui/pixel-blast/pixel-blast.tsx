...
void main(){
  ...
  float cellPixelSize=8.0*pixelSize;vec2 cellId=floor(fragCoord/cellPixelSize);
  vec2 cellCoord=cellId*cellPixelSize;vec2 uv=cellCoord/uResolution*vec2(aspectRatio,1.0);
  ...
  float jitterScale=1.0+(h-0.5)*uPixelJitter;float coverage=bw*jitterScale;float M;
  ...
  if(uEdgeFade>0.0){
    ...
    float fade=smoothstep(0.0,uEdgeFade,edge);M*=fade;
  }
  vec3 color=uColor;
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        const t = threeRef.current as Record<string, unknown>
        ;(t.resizeObserver as ResizeObserver)?.disconnect()
        cancelAnimationFrame(t.raf as number)
        ;(
          (t.quad as THREE.Mesh)?.geometry as THREE.BufferGeometry
        )?.dispose()
        ;(t.material as THREE.ShaderMaterial).dispose()
        ;(t.composer as EffectComposer)?.dispose()
        ;(t.renderer as THREE.WebGLRenderer).dispose()
        ;(t.renderer as THREE.WebGLRenderer).forceContextLoss()
        const el = (t.renderer as THREE.WebGLRenderer).domElement
      ...
      const setSize = () => {
        ...
        if (threeRef.current?.composer)
          (threeRef.current.composer as EffectComposer).setSize(
            renderer.domElement.width,
            renderer.domElement.height,
          )
      ...
      const onPointerDown = (e: PointerEvent) => {
        ...
        const ix = (threeRef.current?.clickIx as number) ?? 0
        ...
        if (threeRef.current)
          threeRef.current.clickIx = ((ix + 1) % MAX_CLICKS) as number
      }
      const onPointerMove = (e: PointerEvent) => {
        ...
        touch.addTouch({
          x: fx / renderer.domElement.width,
          y: fy / renderer.domElement.height,
        })
      }
      ...
      const u = t.uniforms as Record<string, { value: unknown }>
      u.uShapeType.value = SHAPE_MAP[variant] ?? 0
      u.uPixelSize.value =
        pixelSize * (t.renderer as THREE.WebGLRenderer).getPixelRatio()
      ...
      if (transparent)
        (t.renderer as THREE.WebGLRenderer).setClearAlpha(0)
      else (t.renderer as THREE.WebGLRenderer).setClearColor(0x000000, 1)
    }
