...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevColor.current.every((v, i) => v === waveColor[i])) {
      ;(u.waveColor.value as THREE.Color).set(
        waveColor[0],
        waveColor[1],
        waveColor[2],
      )
  ...
  const handlePointerMove = (e: unknown) => {
    ...
    const dpr = gl.getPixelRatio()
    const evt = (e as { nativeEvent?: PointerEvent }).nativeEvent ?? (e as PointerEvent)
    mouseRef.current.set(
      (evt.clientX - rect.left) * dpr,
      (evt.clientY - rect.top) * dpr,
    )
  }
